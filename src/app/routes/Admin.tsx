...
export default function Admin() {
  ...
  const { isAdmin, setTopbarContext } = useOutletContext<{
    setTopbarContext: (ctx: string | null) => void;
    isAdmin: boolean;
  }>();
  ...
  const [activeTab, setActiveTab] = useState<
    "operations" | "ecosystem" | "waitlist" | "survey" | "profiles"
  >("operations");
  ...
  const [usersFilter, setUsersFilter] = useState<"all" | "active" | "inactive">(
    "all",
  );
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        const res = await fetch(`${base}/admin-users-list`, {
  ...
  useEffect(() => {
    const loadEarlyAccess = async () => {
      try {
        ...
        if (earlyStatusFilter !== "all")
          params.set("status", earlyStatusFilter);
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/admin-toggle-user-status`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session?.access_token}`,
        ...
        body: JSON.stringify({
          target_user_id: confirmToggle.userId,
      ...
      setActiveUsers((prev) =>
        prev.map((u) =>
          u.id === confirmToggle.userId
            ? { ...u, deactivated_at: data.deactivated_at }
            : u,
        ),
      );
  ...
  const EN_TO_ES: Record<string, string> = {
    ...
    // keeps_ideas
    "No, I usually keep ideas in my head":
      "No, suelo guardar las ideas en mi cabeza",
    "Yes, in a notes app": "Sí, en una aplicación de notas",
    "Yes, in a productivity tool (Notion, etc.)":
      "Sí, en una herramienta de productividad (Notion, etc.)",
  ...
  function groupByWeek(
    records: EarlyAccessRequest[],
  ): { week: string; count: number; cumulative: number }[] {
        ...
        <div className="admin-tab-content">
          ...
          <div className="admin-two-col">
            ...
            {usersSummary &&
              (() => {
                const inactive =
                  usersSummary.total_users - usersSummary.users_with_content;
                const statusData = [
                  {
                    name: t("admin.active"),
                    value: usersSummary.users_with_content,
                  },
          ...
          <section className="admin-section">
            ...
            <div className="admin-table-wrap">
              <table className="admin-table">
                ...
                <tbody>
                  {activeUsers
                    .slice(
                      (usersPage - 1) * USERS_LIMIT,
                      usersPage * USERS_LIMIT,
                    )
                    .map((user) => (
                      <tr key={user.id}>
                        <td>{user.email}</td>
            ...
            {activeUsers.length > USERS_LIMIT && (
              <div className="admin-pagination">
                ...
                <button
                  disabled={
                    usersPage >= Math.ceil(activeUsers.length / USERS_LIMIT)
                  }
                  onClick={() => setUsersPage((p) => p + 1)}
                  type="button"
                >
        ...
        <div className="admin-tab-content">
          {allEarlyAccess.length === 0 ? (
            ...
            (() => {
              ...
              const platformPieData = sortedPlatforms.map(([name, value]) => ({
                name,
                value,
              }));
              ...
              return (
                <>
                  ...
                  {(() => {
                    const langCounts = allEarlyAccess.reduce(
                      (acc, r) => {
                        const lang =
                    ...
                    const pieData = Object.entries(langCounts).map(
                  ...
                  <section className="admin-section">
                    ...
                    {earlyTotal > EARLY_LIMIT && (
                      <div className="admin-pagination">
                        ...
                        <button
                          disabled={
                            earlyPage >= Math.ceil(earlyTotal / EARLY_LIMIT)
                          }
                          onClick={() => setEarlyPage((p) => p + 1)}
                          type="button"
                        >
        ...
        <div className="admin-tab-content">
          {loadingProfiles ? (
            ...
            (() => {
              ...
              const fields = [
                {
                  label: t("admin.profileFieldDisplayName"),
                  count: profileStats.with_display_name,
                },
                {
                  label: t("admin.profileFieldRole"),
                  count: profileStats.with_role,
                },
                {
                  label: t("admin.profileFieldCountry"),
                  count: profileStats.with_country,
                },
                {
                  label: t("admin.profileFieldLanguage"),
                  count: profileStats.with_language,
                },
                {
                  label: t("admin.profileFieldTimeAvail"),
                  count: profileStats.with_time,
                },
                {
                  label: t("admin.profileFieldSetup"),
                  count: profileStats.with_setup,
                },
              ...
              return (
                <>
                  ...
                  <section className="admin-section">
                    ...
                    <div className="admin-stats">
                      <div className="admin-card stat-card">
                        ...
                        <div className="stat-label">
                          {t("admin.totalProfiles")}
                        </div>
                      </div>
                      <div className="admin-card stat-card">
                        <div className="stat-value admin-highlight">
                          {profileStats.with_display_name}
                        </div>
                        <div className="stat-label">
                          {t("admin.completeProfiles")}
                        </div>
                      </div>
                      <div className="admin-card stat-card">
                        <div className="stat-value admin-highlight">
                  ...
                  <section className="admin-section">
                    <span className="section-label">
                      {t("admin.profileFieldCompleteness")}
                    </span>
                    <div className="admin-card">
                      {fields.map((f) => (
                  ...
                  {(profileStats.language_dist.length > 0 ||
                    ...
                    <div className="admin-two-col">
                      {profileStats.language_dist.length > 0 && (
                        <section className="admin-section">
                          <span className="section-label">
                            {t("admin.profileLanguageDist")}
                          </span>
                          <div className="admin-card admin-card--chart">
                            <WaitlistDonut
                              data={profileStats.language_dist}
                              colors={["var(--primary)", "var(--accent)"]}
                              total={profileStats.language_dist.reduce(
                                (s, d) => s + d.value,
                                0,
                              )}
                            />
                      ...
                      {profileStats.setup_dist.length > 0 && (
                        <section className="admin-section">
                          <span className="section-label">
                            {t("admin.profileSetupDist")}
                          </span>
                          <div className="admin-card admin-card--chart">
                            <WaitlistDonut
                              data={profileStats.setup_dist}
                              colors={PLATFORM_COLORS}
                              total={profileStats.setup_dist.reduce(
                                (s, d) => s + d.value,
                                0,
                              )}
                            />
                  ...
                  {(profileStats.time_dist.length > 0 ||
                    ...
                    <div className="admin-two-col">
                      {profileStats.time_dist.length > 0 && (
                        <section className="admin-section">
                          <span className="section-label">
                            {t("admin.profileTimeDist")}
                          </span>
                          <div className="admin-card admin-card--chart">
                            <WaitlistDonut
                              data={profileStats.time_dist}
                              colors={PLATFORM_COLORS}
                              total={profileStats.time_dist.reduce(
                                (s, d) => s + d.value,
                                0,
                              )}
                            />
                      ...
                      {profileStats.role_dist.length > 0 && (
                        <section className="admin-section">
                          <span className="section-label">
                            {t("admin.profileRoleDist")}
                          </span>
                          <div className="admin-card admin-card--chart">
                            <WaitlistDonut
                              data={profileStats.role_dist}
                              colors={PLATFORM_COLORS}
                              total={profileStats.role_dist.reduce(
                                (s, d) => s + d.value,
                                0,
                              )}
                            />
        ...
        <div className="admin-tab-content">
          {surveyData.length === 0 ? (
            ...
            (() => {
              ...
              // ── Aggregations ──
              const wantsAccess = d.filter(
                (r) =>
                  r.wants_early_access === "Sí" ||
                  r.wants_early_access === "Yes",
              ).length;
              ...
              const seriesLabels: Record<string, string> = {
                "A veces": "A veces",
                Sometimes: "A veces",
                "Sí, con frecuencia": "Sí, frecuentemente",
                "Yes, often": "Sí, frecuentemente",
                Nunca: "Nunca",
                Never: "Nunca",
                "Rara vez": "Rara vez",
                Rarely: "Rara vez",
              ...
              const ideasMulti = [
                {
                  label: t("admin.surveyIdeasRandom"),
                  count: countTrue(d, "ideas_random"),
                },
                {
                  label: t("admin.surveyIdeasNotes"),
                  count: countTrue(d, "ideas_notes"),
                },
                {
                  label: t("admin.surveyIdeasTrends"),
                  count: countTrue(d, "ideas_trends"),
                },
                {
                  label: t("admin.surveyIdeasAudience"),
                  count: countTrue(d, "ideas_audience"),
                },
                {
                  label: t("admin.surveyIdeasAI"),
                  count: countTrue(d, "ideas_ai"),
                },
                {
                  label: t("admin.surveyIdeasSystem"),
                  count: countTrue(d, "ideas_system"),
                },
              ].sort((a, b) => b.count - a.count);

              const knowsMulti = [
                {
                  label: t("admin.surveyKnowsAnalytics"),
                  count: countTrue(d, "knows_what_works_analytics"),
                },
                {
                  label: t("admin.surveyKnowsIntuition"),
                  count: countTrue(d, "knows_what_works_intuition"),
                },
                {
                  label: t("admin.surveyKnowsAudience"),
                  count: countTrue(d, "knows_what_works_audience"),
                },
                {
                  label: t("admin.surveyKnowsUnsure"),
                  count: countTrue(d, "knows_what_works_unsure"),
                },
              ].sort((a, b) => b.count - a.count);

              const contentMulti = [
                {
                  label: t("admin.surveyContentEducational"),
                  count: countTrue(d, "content_educational"),
                },
                {
                  label: t("admin.surveyContentMixed"),
                  count: countTrue(d, "content_mixed"),
                },
                {
                  label: t("admin.surveyContentEntertainment"),
                  count: countTrue(d, "content_entertainment"),
                },
                {
                  label: t("admin.surveyContentReviews"),
                  count: countTrue(d, "content_reviews"),
                },
                {
                  label: t("admin.surveyContentPersonal"),
                  count: countTrue(d, "content_personal"),
                },
                {
                  label: t("admin.surveyContentTutorials"),
                  count: countTrue(d, "content_tutorials"),
                },
              ].sort((a, b) => b.count - a.count);
              ...
              const difficultData = Object.entries(difficultCounts)
                .map(([name, value]) => ({
                  name: difficultLabels[name] ?? name,
                  value,
                }))
              ...
              const valuableLabels: Record<string, string> = {
                "Entender qué es lo que mejor funciona":
              ...
              return (
                <>
                  ...
                  <div className="admin-two-col">
                    ...
                    >
                      <span className="section-label">
                        {t("admin.surveyPlatform")}
                      </span>
                      <div
                        className="admin-card admin-card--chart"
                        style={{ flex: 1 }}
                      >
                        <WaitlistDonut
                          data={platformData}
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                          ...
                          >
                            ...
                            <Tooltip
                              ...
                              formatter={(value) =>
                                [value ?? 0, t("admin.surveyResponses")] as [
                                  number,
                                  string,
                                ]
                              }
                  ...
                  <section className="admin-section">
                    <span className="section-label">
                      {t("admin.surveyCreativeBehavior")}
                    </span>
                    <div className="admin-three-col">
                      <div className="admin-card admin-card--chart">
                        <p className="admin-section-hint">
                          {t("admin.surveyKeepsIdeas")}
                        </p>
                        <WaitlistDonut
                          data={keepsData}
                          colors={[
                          ...
                          <div key={item.label} className="admin-bar-row">
                            ...
                            <div className="admin-bar-track">
                              <div
                                className="admin-bar-fill"
                                style={{
                                  width: `${Math.round((item.count / maxMulti) * 100)}%`,
                                }}
                              />
                            </div>
                            <span className="admin-bar-pct">{item.count}</span>
                        ...
                        <div key={item.name} className="admin-bar-row">
                          ...
                          <div className="admin-bar-track">
                            <div
                              ...
                              style={{
                                width: `${Math.round((item.value / maxDifficult) * 100)}%`,
                              }}
                            />
                          </div>
                  ...
                  <section className="admin-section">
                    ...
                    <div className="admin-card">
                      ...
                      {openPages > 1 && (
                        <div className="admin-pagination">
                          <button
                            disabled={surveyOpenPage === 1}
                            onClick={() => setSurveyOpenPage((p) => p - 1)}
                            type="button"
                          >
                            ‹
                          </button>
                          <span>
                            {surveyOpenPage} of {openPages}
                          </span>
                          <button
                            disabled={surveyOpenPage >= openPages}
                            onClick={() => setSurveyOpenPage((p) => p + 1)}
                            type="button"
                          >
                            ›
            ...
            <p>
              {confirmToggle.action === "deactivate"
                ? t("admin.confirmDeactivateBody", {
                    email: confirmToggle.email,
                  })
                : t("admin.confirmActivateBody", {
                    email: confirmToggle.email,
                  })}
            </p>
